...
int GridCell::get_enemy_count() const {
    return UnitGridFactory::instance().enemy_unit_grid_abstract_factory->precision_map[unit_grid->grid_level]->get_enemy_count(x, z);
}

BlitzUnit *GridCell::find_if(const BlitzUnit* unit, bool (*predicate)(const BlitzUnit*, const BlitzUnit*)) const {
    const UnitGridXXS *grid_xxs = UnitGridFactory::instance().enemy_unit_grid_abstract_factory->grid_xxs;
